...
- (BOOL)control:(NSControl *)control textView:(NSTextView *)textView doCommandBySelector:(SEL)commandSelector;
{
    ...
    if (commandSelector == @selector(cancel:)) {
        // if someone just wants the stupid thing to go away and presses escape, donÕt hinder them
        [self close: control];
        return YES;
    }
    // if someone invokes the default button or switches fields, donÕt override it
    if (commandSelector == @selector(insertNewline:) ||
        commandSelector == @selector(insertTab:) ||
        commandSelector == @selector(insertBacktab:)) return NO;
    [self update: control]; // ...or if they type a navigation key...
    return NO; // we donÕt handle it
}
...
{
    // canÕt rely on dealloc to invalidate the timer, because it retains this object
    [updateTimer invalidate]; updateTimer = nil;
